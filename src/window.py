...
from color import PURPLE_SUPER_DARK, PURPLE_DARK, PURPLE, PURPLE_LIGHT
...
class Window():
    ...
    def __init__(self, windowWidth: int, windowHeight: int,
                 canvasWidth: int,
                 canvasHeight: int, node: MyOwnPeer2PeerNode):
        ...
        self.chess = Chess(self.canvas, canvasWidth,
                           canvasHeight, node.mainWhiteСolor)
        self.fillEntries()
    ...
    def createCanvas(self, canvasWidth: int, canvasHeight: int):
        canvas = Canvas(self.window, width=canvasWidth,
                        height=canvasHeight, bg="white")
    ...
    def createInterface(self, canvasWidth: int, mainWhiteСolor: bool):
        Label(text="ВЫБРАННАЯ ФИГУРА",
              font=("Arial", 16, "bold"), bg=PURPLE_DARK,
              fg="white").place(width=295, height=30, x=canvasWidth + 6, y=0)

        Label(text="X\t\tY", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=250, height=20, x=canvasWidth + 30, y=40)
        ...
        Label(text="Пешка - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=150)
        ...
        Label(text="Конь - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=180)

        self.wHorseEntry = Entry(font=("Arial", 17))
        self.wHorseEntry.place(width=125, height=30,
                               x=canvasWidth + 155, y=180)

        Label(text="Слон - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=210)

        self.wElephantEntry = Entry(font=("Arial", 17))
        self.wElephantEntry.place(
            width=125, height=30, x=canvasWidth + 155, y=210)

        Label(text="Ладья - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=240)
        ...
        Label(text="Ферзь - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=270)

        self.wQueenEntry = Entry(font=("Arial", 17))
        self.wQueenEntry.place(width=125, height=30,
                               x=canvasWidth + 155, y=270)
        ...
        Label(text="Пешка - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=380)
        ...
        Label(text="Конь - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=410)

        self.bHorseEntry = Entry(font=("Arial", 17))
        self.bHorseEntry.place(width=125, height=30,
                               x=canvasWidth + 155, y=410)

        Label(text="Слон - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=440)

        self.bElephantEntry = Entry(font=("Arial", 17))
        self.bElephantEntry.place(
            width=125, height=30, x=canvasWidth + 155, y=440)

        Label(text="Ладья - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=470)
        ...
        Label(text="Ферзь - ", font=("Arial", 16), bg=PURPLE_LIGHT,
              fg=PURPLE_SUPER_DARK).place(
            width=150, height=30, x=canvasWidth + 6, y=500)

        self.bQueenEntry = Entry(font=("Arial", 17))
        self.bQueenEntry.place(width=125, height=30,
                               x=canvasWidth + 155, y=500)
        ...
        colorRadiobutton = Radiobutton(
            ...
            font=("Arial", 16), bg=PURPLE_LIGHT,
            fg=PURPLE_SUPER_DARK, anchor="w",
        ...
        colorRadiobutton = Radiobutton(
            ...
            fg=PURPLE_SUPER_DARK, anchor="w",
            command=lambda: self.changeСolorСhessPieces())
        colorRadiobutton.place(width=150, height=30,
                               x=canvasWidth + 155, y=610)
        ...
        Button(highlightbackground=PURPLE_DARK,
               highlightthickness=30, state=DISABLED).\
            place(width=250, height=30,  x=canvasWidth + 30, y=695)

        infoButton = Button(
            text="Информация о программе", font=("Arial", 16),
            highlightbackground=PURPLE,
            highlightthickness=30, fg=PURPLE_SUPER_DARK,
    ...
    def chooseCell(self, xEvent, yEvent):
        ...
        # игрок поставил шах и мат
            ...
            messagebox.showinfo("Ошибка",
                                "Шах и мат! Игра окончена, победили {:s}!"
                                .format(
                                    "Белые"
                                    if not self.chess.activeWhitePlayer
                                    else "Черные"))
            # закрытие окна
            # self.window.destroy()
        else:
